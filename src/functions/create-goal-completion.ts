...
export async function createGoalCompletion({
  ...
  const result = await db
    ...
    .select({
      desiresWeekLyFrequency: goals.desiresWeekLyFrequency,
  ...
  const { completionCount, desiresWeekLyFrequency } = result[0];

  if (completionCount >= desiresWeekLyFrequency) {
    throw new Error("Goal leready completion this week!");
  }
